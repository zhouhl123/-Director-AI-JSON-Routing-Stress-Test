...
# 【无算力限制配置】：放宽 Token，移除 minimal 压制，允许大模型进行深度逻辑推演
...
# =========================================================
# 故意设置极端的硬性启发式规则，测试模型能否在理解真实意图后“打破”这些规则
DIRECTOR_SYSTEM = """
SYSTEM ROLE: ROUTER.
DEFINITIONS:
...
async def call_director_once(
    ...
    # 【解除封印】：移除了 thinkingConfig，允许模型自由分配 Token 进行深度逻辑推演
...
def generate_comprehensive_word_report(
    ...
    doc = Document()
    doc.add_heading("Adversarial Routing Benchmark Report (Unrestricted)", level=1)
...
async def async_main() -> None:
    ...
    run_root = ROOT_DIR / f"run_adversarial_unrestricted_{timestamp_str()}"
    ensure_dir(run_root)
    ...
    run_config = {
        ...
        "thinking_level": "Unrestricted/Full (No limit)",
        "random_seed": SEED,
    }

    report_path = run_root / "Adversarial_Benchmark_Unrestricted.docx"
